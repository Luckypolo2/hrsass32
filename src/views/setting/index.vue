...
export default {
  ...
  methods: {
    ...
    async assignPerm(id) {
      ...
      this.roleId = id // 存储分配权限的用户ID
      const { permIds } = await getRoleDetail(id) // 获取用户权限信息
      console.log(permIds)
      this.selectCheck = permIds
      this.showPermDialog = true // 打开权限弹窗
    },
    /* 权限弹窗确定取消*/
    async btnPermOk() {
    ...
    btnPermCancel() {
      this.selectCheck = [] // 关闭清空默认选中
      this.showPermDialog = false // 关闭权限弹窗
    }
  }
}
</script>
